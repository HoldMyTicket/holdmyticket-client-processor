...
describe('submit_transaction', () => {
  ...
  test('submits the transaction for fullsteam if transaction processor is fullsteam', (done) => {
    ...
    cc_processor._submit_fullsteam.mockImplementationOnce((card, transaction, cb) =>
      Promise.resolve(successful_transaction_response)
    )
    cc_processor._respond.mockImplementationOnce((err, res, cb) => {
      cb(null, res)
    })
    ...
    cc_processor.submit_transaction(fresh_card_data, fresh_fullsteam_transaction_data, (err, response) => {
      ...
      expect(cc_processor._submit_fullsteam).toHaveBeenCalledWith(
        fresh_card_data,
...
describe('_submit_fullsteam', () => {
  beforeEach(() => {
    ...
    // that hasn't been already mutated from a previous test
    fresh_fullsteam_transaction_data = JSON.parse(JSON.stringify(fullsteam_transaction_data))
    fresh_fullsteam_authentication_key_response_success = JSON.parse(
      JSON.stringify(fullsteam_authentication_key_response_success)
    )
  ...
  test('submits the transaction if payment token already exists', async () => {
    ...
    cc_processor._submit_fullsteam_transaction.mockImplementationOnce((transaction) =>
      Promise.resolve(successful_transaction_response)
    )

    const fullsteam_transaction_with_payment_token = Object.assign({}, fresh_fullsteam_transaction_data, {
      payment_token: fullsteam_payment_token,
    })

    const cc_processor_response = await cc_processor._submit_fullsteam(
      fresh_card_data,
      fullsteam_transaction_with_payment_token
    )
  ...
  test('generates token and submits the transaction if payment token does NOT exist', async () => {
    ...
    cc_processor._get_fullsteam_token.mockImplementationOnce((card, transaction, auth_key) =>
      Promise.resolve(fresh_fullsteam_token_response_success)
    )
    cc_processor._submit_fullsteam_transaction.mockImplementationOnce((transaction) =>
      Promise.resolve(successful_transaction_response)
    )
    ...
    expect(cc_processor._get_fullsteam_token).toHaveBeenCalledWith(
      fresh_card_data,
      fresh_fullsteam_transaction_data,
      fresh_fullsteam_authentication_key_response_success.authenticationKey
    )
  ...
  test('saves card for spreedly if transaction has cc_retain = y', async () => {
    const client_processor_settings_with_spreedly_env_key = Object.assign({}, hmt_client_processor, {
...
describe('_get_fullsteam_auth_key', () => {
  ...
  test('returns fullsteam authentication key', async () => {
    ...
    cc_processor._request.mockImplementationOnce(() =>
      Promise.resolve(fresh_fullsteam_authentication_key_response_success)
    )
...
describe('_get_fullsteam_token', () => {
  ...
  test('returns token and makes request with correct data', async () => {
    ...
    const fullsteam_token_response = await cc_processor._get_fullsteam_token(
      fresh_card_data,
      fresh_fullsteam_transaction_data,
      fresh_fullsteam_authentication_key_response_success.authenticationKey
    )
...
describe('_submit_fullsteam_transaction', () => {
  ...
  test('submits the transaction', async () => {
    ...
    const fullsteam_submit_transaction_response = await cc_processor._submit_fullsteam_transaction(
      fresh_fullsteam_transaction_data
    )
  ...
  test('submits the transaction using charge worker endpoint when charge_workers setting is enabled', async () => {
    ...
    expect(cc_processor._check_charge_worker).toHaveBeenCalledWith(
      successful_create_charge_worker_response.worker_reference
    )
  ...
  test('updates transaction payments array with payment_token if transaction object has payments array', async () => {
    ...
    const expected_payments = [
      { type: 'credit', amount: 10, payment_token: payment_token },
      { type: 'cash', amount: 5 },
    ]
    const expected_fullsteam_transaction_data = Object.assign({}, fresh_fullsteam_transaction_data, {
      payment_token,
...
describe('fullsteam_url', () => {
  ...
  environments.forEach((environment) => {
    test(`returns the correct url for ${environment} environment`, () => {
      const updated_hmt_client_processor_settings = Object.assign({}, hmt_client_processor_settings, {
        env: environment,
      })
