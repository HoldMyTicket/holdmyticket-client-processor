...
var hmt_client_processor = function (settings) {
  ...
  */
  ...
  this._submit_spreedly = async function (card, transaction) {
    ...
    var token_res = await this._get_spreedly_token(
      card,
      transaction.spreedly_environment_key
    );
    ...
    if (
      ...
      if (!token_res) {
        this._add_processing_error(
          "Unable to charge card. Please check Adblocker / Firewall settings and try again."
        );
      }
  ...
  this._get_spreedly_token = async function (
    card,
    spreedly_environment_key,
    cb
  ) {
  ...
  this._submit_spreedly_transaction = async function (transaction, cb) {
    if (transaction.payments)
      transaction.payments = this._update_payments_token(
    ...
    if (this.charge_workers) {
      ...
      if (create_charge_worker_res.status == "error")
        return this._add_processing_error(
          "There was an error processing your transaction."
        );

      var transaction_res = await this._check_charge_worker(
        create_charge_worker_res.worker_reference
      );
    } else {
  ...
  this._submit_fullsteam = async function (card, transaction, cb) {
    ...
    var token_res = await this._get_fullsteam_token(
      card,
      transaction,
      auth_key
    );
  ...
  this._get_auth_key = async function (transaction) {
    ...
    var processor_hash =
      transaction && transaction.processor_hash ?
      transaction.processor_hash :
      "";
    var params = {};
    ...
    var token_res = await this._request({
      url: this.url(this.auth_key_url(), true) +
  ...
  this._get_fullsteam_token = async function (card, transaction, auth_key, cb) {
    ...
    if (!res || !res.isSuccessful) {
      ...
      if (res && res.issuerResponseDetails) {
        ...
        //   msg = "CVC Error: "+CVVResponseDescription; //takes precedence
        //   console.log(res)
        //   console.log(CVVResponseCode)
        // } else {
  ...
  this._submit_authnet = async function (card, transaction, cb) {
    ...
    var token_res = await this._get_authnet_token(
      card,
      transaction,
      authentication_key_res
    );
    ...
    if (
      !token_res ||
      !token_res.opaqueData ||
      !token_res.opaqueData.dataValue
    ) {
      this._add_processing_error(
  ...
  this._get_authnet_token = async function (card, transaction, auth, cb) {
    ...
    var cardData = {};
    cardData.cardNumber = card.payment_method.credit_card.number.replace(
      /\s/g,
      ""
    );
    cardData.expirationDate = this._get_authnet_expiration(
      card.payment_method.credit_card
    );
  ...
  this._get_authnet_expiration = function (card) {
    return (
      (card.month.length == 1 ? "0" : "") +
      card.month +
      (card.year.length == 2 ? "20" : "") +
      card.year
    );
  };
  ...
  this._save_card = async function (card, transaction, processor, ticket_key) {
    ...
    if (processor == "fullsteam") {
      ...
      const token_res = await this._get_fullsteam_token(
        card,
        transaction,
        env_key
      );
  ...
  this._save_card_to_webuser = async function (args) {
    ...
    var card_data = this.card_data ?
      this._format_card_for_save(this.card_data) :
      null;

    if (
      this.card_ticket_key &&
      card_data &&
      this.card_token &&
      this.card_processor
    ) {
  ...
  this._webuser_save_card = async function (card, data, webuser_id, cb) {
    ...
    var spreedly_token_res = await this._get_spreedly_token(
      card,
      data.spreedly_environment_key
    );
    ...
    if (
      save_credit_card_res &&
      save_credit_card_res.status == "ok" &&
      save_credit_card_res.statusText == "OK"
    )
      res = save_credit_card_res; // this will be the successfull res!

    if (
      !save_credit_card_res ||
      !save_credit_card_res.status ||
      save_credit_card_res.status != "ok"
    ) {
    ...
    var fullsteam_token_res = await this._get_fullsteam_token(
      card,
      data,
      env_key
    );
    ...
    if (
      !fullsteam_token_res ||
      !fullsteam_token_res.isSuccessful ||
      !fullsteam_token_res.token
    ) {
    ...
    if (
      !save_additional_card_res ||
      !save_additional_card_res.status ||
      save_additional_card_res.status != "ok"
    ) {
      //do not set error here - we will let if fail silently (we at least saved 1 vault)

      if (!this.errors_processing && !this.errors_internal)
        this._add_internal_error(
          "Fullsteam, Error saving additional credit card"
        );
    ...
    return new Promise((resolve) => {
      const check_charge = async () => {
        var check_charge_worker_res = await this._request({
          url: this.url(
            "shop/carts/get_charge_worker_status/" + worker_reference,
            true
          ),
          withCredentials: true,
        });
        ...
        if (
          ...
          (check_charge_worker_res.worker.status == "waiting" ||
            check_charge_worker_res.worker.status == "running")
        ) {
        ...
        if (
          check_charge_worker_res.status == "ok" &&
          check_charge_worker_res.worker.status == "done"
        ) {
          resolve({
            status: "ok",
        ...
        if (
          check_charge_worker_res.status == "ok" &&
          check_charge_worker_res.worker.status == "terminated"
        ) {
          resolve({
            status: "error",
        ...
        if (
          check_charge_worker_res.status == "ok" &&
          check_charge_worker_res.worker.status == "error"
        ) {
          resolve({
    ...
    return new Promise((resolve, reject) => {
      ...
      if (opts.form_encoded) {
        ...
        if (opts.data.payments) {
          ...
          var stringifiedPayments = this._serializer({
              payments: payments,
            },
            true
          );
      ...
      if (
        ...
        (url.indexOf("submit") > -1 ||
          url.indexOf("get_authentication_key") > -1)
      ) {
  ...
  this._logger = function (url, data, xhr, opts) {
    try {
      ...
      if (
        ...
        d.payment_method.credit_card.verification_value
      )
    ...
    try {
      ...
      if (!d.transaction.processor)
        d.transaction.processor = {
          merch_gateway: transaction.processor_method ?
            transaction.processor_method : null,
        };
